...
describe('Test the restaurant API', () => {
    ...
    test('POST /restaurants/add', async () => {
        const response = await request(app)
            ...
            .send({
                name: 'McDonaldsMcDonalds',
                location: 'New York',
                cuisine: 'Fast Food'
            });
        expect(response.statusCode).toBe(201);
        expect(response.body.name).toBe('McDonaldsMcDonalds');
        expect(response.body.location).toBe('New York');
        expect(response.body.cuisine).toBe('Fast Food');
    });

    test('POST /restaurants/add - Missing required field', async () => {
        const response = await request(app)
            .post('/restaurants/add')
            .send({
                name: '',
    ...
    test('POST /restaurants/add - Name length validation', async () => {
        const response = await request(app)
            .post('/restaurants/add')
            .send({
                name: 'Res',
        ...
        expect(response.body.errors).toBeDefined();
        const errorMessages = response.body.errors.map(err => err.msg);
        expect(errorMessages).toContain('Name should have 10-30 characters');
    })
